...
derecho = "VIDA"
vectoFile = "/Users/amgiraldov/OneDrive - Consejo Superior de la Judicatura/Documentos/2021/EntrenamientoDerechos/output/clasHashNSte_First1km.joblib"

t0 = time()
clasificacion = pd.read_excel(dataBaseTag)
duration = time() - t0
print("done in  %0.3f" % (duration) ,"seconds")


clasificacion["newText"] = [x[:10000] for x in clasificacion['text']]

data_train, data_test, train_labels, test_labels = train_test_split(clasificacion["newText"], 
                                                                    clasificacion[derecho],
                                                                    stratify = clasificacion[derecho],
                                                                    test_size = 0.20)
...
t0 = time()
vectorizer = joblib.load(vectoFile)
